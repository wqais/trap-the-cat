...
public class Game implements Runnable {
	...
	public void run() {
		...
		/*
		 * for (int i = 0; i < NUMBER_OF_BLOCKED_CELL; i++) { int x =
		 * random.nextInt(10); int y = random.nextInt(10);
		 * 
		 * Cell cell = new Cell(x, y); trapper.write(cell); cat.write(cell);
		...
		while (true) {
			...
			if (cell.getX() == -1 && cell.getY() == -1) {
				if (turn.getTurn() == 1)
					System.out.println("Trapper has won!");
				else {
					String path = "D:/eclipse-workspace/TrapTheCat/src/resources/cat_win.mp3";
					Media media = new Media("https://www.learningcontainer.com/wp-content/uploads/2020/02/Kalimba.mp3");
